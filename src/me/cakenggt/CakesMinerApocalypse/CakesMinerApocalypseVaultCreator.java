...
public class CakesMinerApocalypseVaultCreator implements Listener {
    ...
    public void vaultCreator(ChunkLoadEvent event) throws IOException {
        p.EWorlds = p.getConfig().getStringList("AlternateWorldNames");
        World worldd = event.getWorld();
        ...
        if (randomB <= 1) {
            if(p.EWorlds.contains(worldd.getName())) {
                Location start = new Location(worldd, x + 8, y, z + 8);
        ...
        if (randomA <= 1) {
            if(p.EWorlds.contains(worldd.getName())) {
                Location start = new Location(worldd, x + 8, y, z + 2);
                placeShelter(start);
            }
        }
    }
	...
	public void placeShelter(Location start) {
        ...
        if(p.EWorlds.contains(worldd.getName())) {
		    ...
		    Location end = new Location(worldd, start.getX() + 8, start.getY() + 7, start.getZ() + 11);
		    int xIteration = 0;
		    int yIteration = 0;
		    ...
		    end = new Location(start.getWorld(), start.getX() + 4, start.getY() + 3, start.getZ() + 7);
		    ...
		    Chest chest = (Chest) start.getBlock().getRelative(4, 0, 3).getState();
		    Inventory chestInventory = chest.getInventory();
		    ...
		    start.getBlock().getRelative(0, 0, 7).setType(Material.ENCHANTMENT_TABLE);
		    start.getBlock().getRelative(1, 0, 7).setType(Material.BREWING_STAND);
		    start.getBlock().getRelative(2, 0, 7).setType(Material.WORKBENCH);
		    start.getBlock().getRelative(3, 0, 7).setType(Material.FURNACE);
		    Furnace furnace = (Furnace) start.getBlock().getRelative(3, 0, 7).getState();
